...
struct thread * get_child_process (tid_t tid)
{
  ...
  struct list_elem * c1 = list_begin(&thread_current()->child);
  struct thread * t1;
 while(c1 != list_end(&thread_current()->child))
 {
   t1 = list_entry(c1,struct thread,child_elem);
   if(t1->tid == tid)
   {  return (t1);}
   c1 = list_next(c1);
 }
